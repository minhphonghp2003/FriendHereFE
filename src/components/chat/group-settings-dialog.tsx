...
export function GroupSettingsDialog({
  ...
  const handleRemove = async (member: ConversationMemberDto) => {
    if (!conversationId) return;
    if (!window.confirm(`Xóa ${member.userName} khỏi nhóm? Họ sẽ ngừng nhận tin nhắn từ nhóm.`)) return;
  ...
  const handleDeleteGroup = async () => {
    if (!conversationId) return;
    if (!window.confirm("Xóa nhóm này? Cuộc trò chuyện, thành viên và tin nhắn sẽ bị xóa vĩnh viễn. Hành động này không thể hoàn tác.")) return;
    try {
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        ...
        {view === "add" ? (
          <div className="flex flex-col gap-2">
            <p className="text-sm text-muted-foreground">Chọn bạn bè để thêm vào nhóm</p>
            <div className="max-h-72 overflow-y-auto rounded-lg border border-border">
              {friendsLoading ? (
                ...
                <p className="py-8 text-center text-sm text-muted-foreground">Không còn bạn bè nào để thêm</p>
              ) : (
                friends.map((friend) => (
                  <div key={friend.id} className="flex items-center gap-3 px-3 py-2.5">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-muted text-sm font-bold text-muted-foreground">
                      {friend.otherUserImage?.thumbUrl ? (
                        <img src={friend.otherUserImage.thumbUrl} alt="" className="h-full w-full object-cover" />
                      ) : (
                        getNameDisplay(friend.otherUserName ?? "")
                      )}
                    </div>
                    <p className="flex-1 truncate text-sm font-medium">{friend.otherUserName}</p>
                    <Button type="button" size="sm" disabled={addingMember} onClick={() => handleAdd(friend)}>
          ...
          <div className="flex flex-col gap-5">
                    ...
                    <div key={member.userId} className="flex items-center gap-3 px-3 py-2.5">
                      ...
                      {isHost && member.role === ConversationMemberRole.Member && (
                        <button
                          ...
                          className="shrink-0 rounded-full p-1.5 text-muted-foreground hover:bg-red-500/10 hover:text-red-500 disabled:opacity-50"
            ...
            {!loadingMembers && !membersError && members.length > 0 && (
              <div className="flex flex-col gap-2">
                {isHost ? (
                  <Button type="button" variant="outline" className="w-full text-red-600 hover:bg-red-500/10" onClick={handleDeleteGroup}>
                    <Trash2 className="h-4 w-4" />
                    Xóa nhóm
                  </Button>
                ) : (
                  <Button type="button" variant="outline" className="w-full text-red-600 hover:bg-red-500/10" disabled={leaving} onClick={handleLeave}>
